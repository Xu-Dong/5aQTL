...
# thresh <- 0.05
out <- readRDS("./fastqtl_to_mash_output/SNP_5aQTL_file_list.mash.rds")
maxbeta <- out$strong.b
maxz    <- out$strong.z
cname <- colnames(maxbeta) %>% gsub(".txt", "", .)


lfsr <- read.table("./SNP_5aQTL.brain.lfsr.txt", header = T)

out2 <- readRDS("./SNP_5aQTL.nominal.mash.final.results.rds")
...
sigmat <- (lfsr <= thresh)
nsig   <- rowSums(sigmat)
c <- hist((het.func(het.norm(effectsize=pm.mash.beta[nsig>nsigthresh,]),threshold=hetthresh)),main="",xlab="",ylab="Proportion of eQTL",
          breaks=0.5:13.5,col=rgb(1,0.5,0.5,alpha=0.8),freq=FALSE, xaxt="n",cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5,border = NA)
axis(1,at = seq(1, 13, by=1),labels = c(1:13))
mtext("All Tissues")
...
a <- hist(sign.func(het.norm(effectsize=pm.mash.beta[nsig>nsigthresh,])),main="",xlab="",
          breaks=0.5:13.5,col="black",freq=FALSE,xaxt="n",ylim=c(0,1)) 
...
saveRDS(df_all,file = "Sharing.hist.5aQTL.brain.RDS")
